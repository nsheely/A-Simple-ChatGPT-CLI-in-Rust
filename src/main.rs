...
async fn chat(
    ...
    if let Ok(chat_response) = chat_response {
        ...
        Err(CustomError::ParseError("Error parsing the API response".to_string()))
    }
}
...
async fn main() {
    ...
    if let Some(input) = matches.value_of("input") {
        single_message_mode(&client, &api_key, api_url, model, input.trim(), &mut messages).await;
    } else if matches.is_present("interactive") { // If the interactive flag is present, use interactive mode.
